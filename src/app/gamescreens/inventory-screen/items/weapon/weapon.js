...
export default class Weapon {
  constructor(name, element1, element2 = elements[4], element3 = elements[4], element4 = elements[4]) {
    this.name = name;
  ...
  static enhanceWeapon(weapon, crystal) {
    ...
    if (crystal.crystalElement) {
      if (crystal.rarity === 'Basic' && isSameElement(weapon.weaponElements[0].element, elements[4])) {
        weapon.weaponElements[0] = crystal.crystalElement;
        return;
      }
      if (crystal.rarity === 'Rare' && isSameElement(weapon.weaponElements[1].element, elements[4])) {
        weapon.weaponElements[1] = crystal.crystalElement;
        return;
      }
      if (crystal.rarity === 'Epic' && isSameElement(weapon.weaponElements[2].element, elements[4])) {
        weapon.weaponElements[2] = crystal.crystalElement;
        return;
      }
      if (crystal.rarity === 'Legendary' && isSameElement(weapon.weaponElements[3].element, elements[4])) {
