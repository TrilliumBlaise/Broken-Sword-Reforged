...
import {
  addToInventory,
  damageDurabiliy,
} from '../inventory-screen/ItemAPI.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(function () {
    ...
    document.querySelector('.text').innerHTML =
      'You look around in search of battle.';
    fillTheSky();
    setTimeout(function () {
      document.querySelector('.text').innerHTML = '';
      spawn();
...
document.querySelector('.attack-button').addEventListener('click', () => {
  ...
  if (weaponDrop[1]) {
    ...
    document.querySelector(
      '.battle-text'
    ).innerHTML = `Your weapon has broken.<br>As the dust settles you find ${weaponDrop[1]}`;
  }
  setTimeout(function () {
    ...
    document.querySelector(
      '.enemy-hp'
    ).innerHTML = `HP: ${enemy.hp}/${enemy.maxHP}`;
    if (player.hp === 0) {
      dead();
    }
    if (enemy.hp === 0) {
...
document.querySelector('.inventory-button').addEventListener('click', () => {
  ...
  document
    .querySelector('.inventory-screen')
    .classList.replace('fadeOut', 'fadeIn');
  document.querySelector(
    '.inventory-screen'
  ).innerHTML += `<iframe src= '../inventory-screen/inventory-screen.html' frameborder= '0'></iframe>`;
  ...
  document.querySelector('.close-button').addEventListener('click', () => {
    ...
    document.querySelector(
      '.inventory-screen'
    ).innerHTML = `<button class="close-button">X</button>`;
    player = CurrentPlayerAPI.read();
  });
});
...
document.querySelector('.run-button').addEventListener('click', () => {
  ...
  document.querySelector(
    '.battle-text'
  ).innerHTML = `You have tried to run away, but the creature caught up with you.<br>`;
...
document.querySelector('.continue-button').addEventListener('click', () => {
  ...
  document.querySelector('.text').innerHTML =
    'You press forward, in search of battle.';
  fillTheSky();
  setTimeout(() => {
    document.querySelector('.text').innerHTML = '';
    spawn();
...
document.querySelector('.return-button').addEventListener('click', () => {
  document.querySelector('.text').innerHTML =
    'Finding nothing else, you return to the village.';
  CurrentPlayerAPI.save(player);
  setTimeout(() => {
    window.location.href = '../village-screen/village-screen.html';
  }, delayInMilliseconds);
...
function spawn() {
  ...
  if (boolean) {
    ...
    if (enemy.enemyElement.element[0].match(/^[aeiou].*/i)) {
      document.querySelector(
        '.battle-text'
      ).innerHTML = `You have encountered an ${enemy.enemyElement.element} ${enemy.name} prepare for battle!`;
    }
    if (!enemy.enemyElement.element[0].match(/^[aeiou].*/i)) {
      document.querySelector(
        '.battle-text'
